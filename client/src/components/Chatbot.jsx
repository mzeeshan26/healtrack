    ...
    <>
      ...
      {!isOpen && (
        <div 
          ...
          className={`!fixed w-20 h-20 rounded-full shadow-[0_10px_40px_rgba(16,185,129,0.4)] transition-all z-[9999] group cursor-grab active:cursor-grabbing flex items-center justify-center
            ${isDragging ? 'scale-110 shadow-[0_20px_60px_rgba(16,185,129,0.7)] rotate-3' : 'animate-float hover:scale-105'}
          `}
        >
          ...
          {/* Radar Pulse Effect */}
          <div className="absolute inset-0 bg-white rounded-full animate-ping opacity-20 pointer-events-none" style={{ animationDuration: '3s' }}></div>
      ...
      {/* Chat Window Drawer */}
      {isOpen && (
        <div className="!fixed top-0 right-0 w-[100vw] sm:w-[450px] h-screen bg-white/80 backdrop-blur-xl border-l border-white/60 shadow-[-10px_0_40px_rgba(0,0,0,0.1)] flex flex-col z-[9999] flex-shrink-0 animate-in slide-in-from-right-8 duration-300">
          
          {/* Header */}
          <div className="bg-gradient-to-r from-emerald-500 to-cyan-500 p-6 border-b border-white/60 flex justify-between items-center text-white relative">
            ...
            <button onClick={() => setIsOpen(false)} className="hover:bg-white/20 p-1.5 rounded-lg text-white transition-colors relative z-10">
               <X size={20} />
            </button>
          </div>

          {/* Mode Selector */}
          <div className="flex border-b border-white/60 text-sm font-semibold bg-white">
            <button 
              onClick={() => setMode('medical_assistant')}
              className={`flex-1 py-3.5 flex items-center justify-center gap-2 transition-all ${mode === 'medical_assistant' ? 'text-teal-600 bg-teal-50 shadow-inner border-b-[3px] border-teal-500' : 'text-gray-500 hover:bg-gray-50'}`}
            >
            ...
            {canUsePatientData && (
              <button 
                ...
                className={`flex-1 py-3.5 flex items-center justify-center gap-2 transition-all ${mode === 'patient_data' ? 'text-teal-600 bg-teal-50 shadow-inner border-b-[3px] border-teal-500' : 'text-gray-500 hover:bg-gray-50'}`}
              >
          ...
          {/* Messages Area */}
          <div className="flex-1 p-6 overflow-y-auto bg-gradient-to-b from-white/30 to-white/10 space-y-6 custom-scrollbar relative">
            {messages.map((m, i) => (
                ...
                <div className={`max-w-[85%] p-3.5 shadow-md font-medium text-sm transition-all
                  ${m.sender === 'user' 
                    ...
                    : m.isError 
                      ...
                      : 'bg-white/90 backdrop-blur-sm text-textPrimary rounded-2xl rounded-tl-sm border border-white'}
                `}>
                  {m.text}
                </div>
                <span className="text-[10px] font-bold text-gray-400 mt-1.5 mx-2 uppercase">
            ...
            {loading && (
              <div className="flex items-start">
                <div className="bg-white/90 backdrop-blur-sm p-4 rounded-2xl rounded-tl-sm border border-white text-sm flex gap-1.5 shadow-sm">
          ...
          {/* Input Area */}
          <form onSubmit={handleSend} className="p-5 border-t border-white/60 bg-white/60 backdrop-blur-xl flex gap-3 relative pb-8 sm:pb-5">
            <input 
              type="text" 
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Ask for clinical assessments..."
              className="flex-1 px-5 py-3.5 bg-white/90 border border-white focus:outline-none focus:ring-2 focus:ring-teal-400 rounded-xl text-sm font-medium shadow-inner transition-all placeholder:text-gray-400"
              disabled={loading}
            />
            <button 
              type="submit" 
              disabled={loading || !input.trim()}
              className="p-3 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-xl shadow-lg hover:shadow-xl hover:opacity-90 disabled:opacity-50 disabled:hover:shadow-none transition-all flex items-center justify-center shrink-0 w-14 h-14"
            >
